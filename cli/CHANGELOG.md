...
- [#58](https://github.com/logaretm/teley/pull/58) [`fe44024`](https://github.com/logaretm/teley/commit/fe440249ae3923b88a8547ad7b3808aea2ff2b3e) Thanks [@logaretm](https://github.com/logaretm)! - Add a metrics view. The relay has been broadcasting `metric_update` all along and the CLI dropped it on the floor, so counters, gauges, histograms, and sets now land in the TUI, `--json`, `--local`, and a `list_metrics` MCP tool alongside traces and logs. `←`/`→` cycles three views rather than toggling two.
